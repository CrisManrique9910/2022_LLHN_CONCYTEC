...
import sys
from my_funcs import my_arctan, get_scale
...
for tev in tevs[:1]:
    ...
    for type in types[:]:
        for card in cards[:]:
            ...
            plt.xlabel('Photon 1')
            plt.ylabel('Photon 2')
            plt.title(f'Filtro {ip}: Eta menor a 2.37 \n{events} eventos')
            ...
            plt.xlabel('Photon 1')
            plt.ylabel('Photon 2')
            plt.title(f'Filtro {ip}: Ninguno en el intersec y >=1 en el barrel\n{events} eventos')
            plt.savefig(destiny_filters+f'filter{ip}_2D.png')
            #plt.show()
            plt.close()
            ...
            maxet = np.maximum(maxet,100*vmax/events)
            ...
            for j in range(juice.shape[0]):
                for i in range(juice.shape[1]):
                    if big_mask[j,i]:
                        ...
                        ax.text(i,j,f'{number}',horizontalalignment="center",
                                verticalalignment="center",color=c)
            plt.xlabel('Photon 1\'s ET [GeV]')
            plt.ylabel('Photon 2\'s ET [GeV]')
    ...
    for card in list(juices.keys())[:]:
        for type in list(juices[card].keys())[:]:
            ...
            ax.text(0.05,0.76,f"{names[type]}\n$\mathregular{{M_{{h}}}}$ "
                              f"= {mass[card]} GeV\n{round(get_scale(tev,type,mass[card])*events)} Events",
